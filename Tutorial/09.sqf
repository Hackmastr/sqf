...
for "_idx" from 0 to (count tienda)-1 do {
	if ((tienda select _idx select 0) == _que) exitWith {
		...
		_indice = _idx; // eso lo usaré más tarde para más tarde poder encontrar mi item dentro del array tienda
	};
};
...
if (_param) then {
	...
	_stock = _stock - 1; // esto sólo lo uso para el mensaje final
	
	dinero = dinero - _precio;
	missionNamespace setVariable [_que, (missionNamespace getVariable _que) + 1];
	(tienda select _indice) set [2, (tienda select _indice select 2) - 1];
	
	hint format ["Gracias por comprar %1 por %2€ (quedan %3 unidades)", _que, _precio, _stock];
} else {
	...
	hint format ["Gracias por vender %1 por %2€ (quedan %3 unidades)", _que, _precio, _stock];
};
...
Muchas veces se pueden sustituír los {} por "" o por ''.
...
Pero tranqui, que el arma te dará error y al menos sabrás porqué.
