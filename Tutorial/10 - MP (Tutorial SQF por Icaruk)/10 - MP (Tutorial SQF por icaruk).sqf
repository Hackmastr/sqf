...
// Guardamos la misión con el segundo icono de disquette (o ctrl + shift + s)
// y en lugar de guardar como "User mission" lo hacemos como "Export to multiplayer missions".
...
// Os borráis la variable valor = nil y probamos otra cosa.
// Ejecutáis en SERVER:
...
- El stock iría con publicVariable, no os comáis la cabeza publicVariableServer y publicVariableClient

Cada vez que tengáis que testear, tenéis que guardar la misión como hemos hecho antes: "export to multiplayer missions".
Mirar bien que no esté en uso porque no se podrá sobreescribir, es decir: tenéis que cerrar el dedicado, para que deje de
...
tioVendedor addAction [
	...
	'[argumentos] execVM "tienda.sqf"; ', // código a ejecutar, fijaros en cómo uso las comillas, probar a usarlas de otras formas.
	...
	"" // condición, por ejemplo: "(side player == WEST) AND ((player distance tioVendedor) < 3)". Ojo, se comprueba a cada frame, por eso siempre lo dejo vacío.
];

// Aquí podéis ver addAction más extensamente https://community.bistudio.com/wiki/addAction
// No es necesario poner todos los parámetros, los parámetros que omitáis usarán sus valores por defecto, que podéis veremos
